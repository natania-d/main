...
The recurring tasks function is facilitated by a `Schedule` interface, which resides inside `ModelManager`. It allows selected tasks to be recurring over a time period using a regular time interval such as weekly or monthly. The Schedule interface is as such:
...
The user executes a command `repeat weekly 5`, to recur the 5th task in the organizer weekly.
